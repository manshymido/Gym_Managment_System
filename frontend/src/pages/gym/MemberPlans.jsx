...
import { getAllMemberPlans, createMemberPlan, updateMemberPlan, deleteMemberPlan } from '../../services/gymApi';
...
const MemberPlans = () => {
  ...
  const fetchPlans = async () => {
    try {
      const response = await getAllMemberPlans();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingPlan) {
        await updateMemberPlan(editingPlan._id, data);
      } else {
        await createMemberPlan(data);
      }
      fetchPlans();
      resetForm();
    } catch (error) {
  ...
  const handleDeleteConfirm = async () => {
    if (deletePlanId) {
      try {
        await deleteMemberPlan(deletePlanId);
        fetchPlans();
  ...
  return (
    <GymLayout>
      <div style={{ width: '100%' }}>
        <PageHeader
          title="باقات الأعضاء"
          subtitle="إدارة باقات الاشتراك للأعضاء"
          action="add"
